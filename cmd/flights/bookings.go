...
// Booking is a flat record: one PNR + one passenger.
type Booking struct {
	PNR   string
	First string
	Last  string
...
func LoadBookings() ([]Booking, error) {
	...
	// Support old format with extra fields (password, member_id, etc.) by using
	// a permissive intermediate type during decode.
